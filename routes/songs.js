...
router.post("/", async (req, res) => {
    ...
    if (!songInfo.title || typeof songInfo.title != "string") {
        res.status(400).json({error: 'You must provide a title of the album as string'});
        return;
    }

    if (!songInfo.user || typeof songInfo.user != "string" || !ObjectId.isValid(songInfo.user)) {
        res.status(400).json({error: 'You must provide id of the artist as a string or an object id'});
        return;
        }
    ...
    if (!songInfo.genre || !Array.isArray(songInfo.genre)) {
        res.status(400).json({error: 'You must provide a array of genre in the song'});
        return;
        }
